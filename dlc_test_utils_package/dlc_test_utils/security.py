...
class AllowListFormatVulnerabilityForEnhancedScan:
    ...
    def __eq__(self, other):
        ...
        if dlc_test_utils.check_if_two_dictionaries_are_equal(
            dataclasses.asdict(self.package_details),
            dataclasses.asdict(other.package_details),
            ignore_keys=["version"],
        ):
            return dlc_test_utils.check_if_two_dictionaries_are_equal(
...
class ScanVulnerabilityList:
    ...
    def get_sorted_vulnerability_list(self):
        ...
        for key, list_of_complex_types in copy_dict.items():
            uniquified_list = dlc_test_utils.uniquify_list_of_complex_datatypes(list_of_complex_types)
    ...
    def __add__(self, other):
        ...
        if not all_vulnerabilities:
            return None
        union_vulnerabilities = dlc_test_utils.uniquify_list_of_complex_datatypes(all_vulnerabilities)
...
def get_ecr_scan_allowlist_path(image_uri):
    dockerfile_location = dlc_test_utils.get_dockerfile_path_for_image(image_uri)
    ...
    if (
        ...
        and dlc_test_utils.is_covered_by_ec2_sm_split(image_uri)
        and dlc_test_utils.is_ec2_sm_in_same_dockerfile(image_uri)
    ):
        if dlc_test_utils.is_ec2_image(image_uri):
...
def get_target_image_uri_using_current_uri_and_target_repo(
    ...
    (
        ...
        original_image_tag,
    ) = dlc_test_utils.get_repository_and_tag_from_image_uri(image)
    if append_tag:
...
def _invoke_lambda(function_name, payload_dict={}):
    ...
    """
    lambda_client = boto3.client("lambda", region_name=dlc_test_utils.DEFAULT_REGION)
...
def conduct_failure_routine(
    ...
    apt_upgrade_list_filename = (
        f"apt-upgrade-list-{dlc_test_utils.get_processor_from_image_uri(image)}.txt"
    )
    ...
    ## TODO: Make the conditions below as if dlc_test_utils.is_canary_context() and dlc_test_utils.is_time_for_invoking_ecr_scan_failure_routine_lambda() and os.getenv("REGION") == dlc_test_utils.DEFAULT_REGION:
    ## to make sure that we just invoke the ECR_SCAN_FAILURE_ROUTINE_LAMBDA once everyday
    if dlc_test_utils.is_canary_context() and os.getenv("REGION") == dlc_test_utils.DEFAULT_REGION:
        # boto3.Session().region_name == dlc_test_utils.DEFAULT_REGION helps us invoke the ECR_SCAN_FAILURE_ROUTINE_LAMBDA
        # from just 1 account
        _invoke_lambda(
            function_name=dlc_test_utils.ECR_SCAN_FAILURE_ROUTINE_LAMBDA, payload_dict=message_body
        )
...
def fetch_other_vulnerability_lists(image, ecr_client, minimum_sev_threshold):
    ...
    new_image_uri_for_upgraded_image = get_target_image_uri_using_current_uri_and_target_repo(
        image,
        target_repository_name=dlc_test_utils.UPGRADE_ECR_REPO_NAME,
        target_repository_region=os.getenv("REGION", dlc_test_utils.DEFAULT_REGION),
        append_tag="upgraded",
    )
