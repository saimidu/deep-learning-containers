...
from dlc_build_utils.config import is_build_enabled
from dlc_build_utils.safety_report_generator import SafetyReportGenerator
...
def fetch_dlc_images_for_test_jobs(images, use_latest_additional_tag=False):
    ...
    :param images: list
    :param use_latest_additional_tag: bool True if image to be pushed for tests has multiple tags
    :return: dictionary
    """
...
def set_test_env(images, use_latest_additional_tag=False, images_env="DLC_IMAGES", **kwargs):
    ...
    :param use_latest_additional_tag: bool True if image to be tested has multiple tags
...
def get_safety_ignore_dict(image_uri, framework, python_version, job_type):
    """
    Get a dict of known safety check issue IDs to ignore, if specified in file
    ...
    ignore_data_file = os.path.join(
        get_cloned_folder_path(), "data", "ignore_ids_safety_scan.json"
    )
    ...
    # Find common vulnerabilities and add it to the ignore dict
    common_ignore_list_file = os.path.join(
        get_cloned_folder_path(), "data", "common-safety-ignorelist.json"
    )
...
def generate_safety_report_for_image(image_uri, image_info, storage_file_path=None):
    ...
    :param image_uri: str, consists of f"{image_repo}:{image_tag}"
    :param image_info: dict, should consist of 3 keys -
                        "framework", "python_version", and "image_type"
...
def get_label_prefix_customer_type(image_tag):
    ...
    # Older images are not tagged with ec2 or sagemaker.
    # Assuming that lack of ec2 tag implies sagemaker.
    return "sagemaker"
